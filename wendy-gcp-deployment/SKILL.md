...
6. **dev/prod split by default.** Default branch deploys to dev; semver tags (`v*`) deploy to prod. Skip the split only with a stated good reason. Today each repo keeps every env in one GCP project, split by resource naming (`-dev` / `-prod` suffixes): pki-core `wendy-pki-secure`, cloud `cloud-c7e56` (incl. `legacy-prod`), wendy-auth `wendy-auth` (its envs are `test` / `prod`). Source: `gcp:project` (wendy-auth foundation: `projectId`) in each `Pulumi.<stack>.yaml`, foundation and platform. Every `v*` tag is a GitHub Release with generated notes. pki-core, wendy-auth and cloud additionally split each env into a **foundation** stack (network, DNS zones, custom IAM roles; applied by hand) and a **platform** stack (applied by CI, whose deploy SA is never widened) — see `references/iam-polp.md`.
