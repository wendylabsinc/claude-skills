...
The sharpest residual is a cloud that lies **consistently** to both the device and pki-core about the tail (claiming "row 11 is the latest" to everyone) — since pki-core learns the head only via cloud, closing it needs a second ingestion path cloud doesn't control. This is **partially closed**: operator tooling posts signed submission receipts directly to pki-core, which refuses to notarize a new head while a target's receipt sits unacknowledged past a grace window, and a device can query pki-core directly for its own receipt lower bound. It's closed for any request that produced a submission receipt; it remains open otherwise, pending wendy-CLI actually posting the receipt at submission time and cloud acknowledging receipt hashes on notarization. A witness in a fully separate trust domain stays the **deferred, higher-assurance option** that would close it unconditionally (D8, §10 item 4).
